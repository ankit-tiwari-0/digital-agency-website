window.addEventListener("load", function() {
    ...
    mm.add("(max-width:600px)", () => {
        ...
        // ===== Section 2 (Services) =====
        let tl2 = gsap.timeline({
            scrollTrigger: { trigger: ".section2", start:"top 90%", end:"top 20%", scrub:0.6 }
        });
        tl2.from(".section1boyyom img", { y:20, opacity:0, duration:0.4, stagger:0.1 });
        tl2.from(".services h3", { x:-20, opacity:0, duration:0.3 });
        tl2.from(".services p", { x:20, opacity:0, duration:0.3 });

        // Animate the 4 cards separately but smoother
        tl2.from(".elem.left", { x:-100, opacity:0, duration:0.7, stagger:0.15 });
        tl2.from(".elem.right", { x:100, opacity:0, duration:0.7, stagger:0.15 }, "-=0.5");
        tl2.from(".elem.down", { y:80, opacity:0, duration:0.7, stagger:0.15 }, "-=0.4");
